#
# DESCRIPTION
#  Array::Compare.pm is a Perl object that compares arrays.
#
# AUTHOR
...
# Default values.
#
# Sep - is the value that is used to separate fields when the array
# is joined into a string. It should be a value which doesn't appear
# in your data. Default is '^G'.
#
# WhiteSpace - Flag that indicates whether or not whitespace is
# significant in the comparison. If this value is true then all multiple
...
#
# DefFull - Flag which indicates whether the default comparison is
# simple (just returns true if the arrays are the same or false if
# they're not) or full (returns an array containing the indexes of
# the columns that differ). Default is 0 (simple comparison).
#
my %_defaults = (Sep => '^G',
		 WhiteSpace => 1,
                 Case => 1,
...
# Object constructor.
#
# Optional arguments are hash values which will override the
...
#
# Very simple comparison. Just checks the length of the array is
# the same.
#
sub compare_len {
  my $self = shift;
...
#
# Compare the values in two arrays and return a flag indicating
# whether or not the arrays are the same.
#
...
# of columns that differ (zero if the arrays are the same). In array
# context returns an array containing the indexes of the columns that
